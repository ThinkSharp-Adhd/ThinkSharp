...
function loadQuestion() {
  ...
  ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'].forEach((label, i) => {
    ...
    c.setAttribute('aria-label', `Select ${label}`);
...
function checkAllTasksCompleted() {
  ...
  if (allCompleted) {
    ...
    refreshButton.setAttribute('aria-label', 'Get new tasks');
...
function showResult() {
  ...
  quizState.tasks.forEach((task, i) => {
    ...
    if (task.action === 'meditation') {
      actionHtml = `
        <button id="meditation-start-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Meditation completed' : 'Start meditation'}">${isCompleted ? 'Completed' : 'Start'}</button>
        <div id="meditationTimer${i}" class="hidden ml-[1vw]">
          <span id="meditationDisplay${i}" class="text-[0.9rem]"></span>
          <button id="meditation-stop-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-red-500 text-white rounded hover:bg-red-600" aria-label="Stop meditation">Stop</button>
        </div>`;
    } else if (task.action === 'pomodoro') {
      actionHtml = `
        <button id="pomodoro-start-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Pomodoro completed' : 'Start Pomodoro'}">${isCompleted ? 'Completed' : 'Start'}</button>
        <div id="pomodoroTimer${i}" class="hidden ml-[1vw]">
          <span id="pomodoroDisplay${i}" class="text-[0.9rem]"></span>
          <button id="pomodoro-stop-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-red-500 text-white rounded hover:bg-red-600" aria-label="Stop Pomodoro">Stop</button>
        </div>`;
    } else if (task.action === 'notepad') {
      actionHtml = `
        <div class="ml-[1vw] w-full">
          <textarea id="notepad${i}" class="w-full max-w-[300px] h-[80px] resize-none p-[0.5rem] border rounded text-[0.9rem] dark:bg-gray-700 dark:text-gray-200" placeholder="Jot down your to-dos..." aria-label="Notepad for to-dos"></textarea>
          <div class="flex space-x-[1vw] mt-[0.5vh]">
            <button id="notepad-save-${i}" class="py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Notepad saved' : 'Save notepad'}">${isCompleted ? 'Completed' : 'Save'}</button>
            <button id="notepad-clear-${i}" class="py-[0.5rem] px-[1rem] bg-gray-500 text-white rounded hover:bg-gray-600" aria-label="Clear notepad">Clear</button>
          </div>
      ...
      actionHtml = `
        ...
        <button id="workspace-complete-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Workspace setup completed' : 'Mark workspace setup as completed'}">${isCompleted ? 'Completed' : 'Mark as Completed'}</button>`;
    } else if (task.action === 'checklist') {
      actionHtml = `
        <div class="ml-[1vw]">
          <input id="checklistInput${i}" class="p-[0.5rem] border rounded text-[0.9rem] dark:bg-gray-700 dark:text-gray-200" placeholder="Add task..." aria-label="Add task to checklist">
          <button id="checklist-add-${i}" class="ml-[0.5vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Checklist completed' : 'Add task to checklist'}">${isCompleted ? 'Completed' : 'Add'}</button>
          <div id="checklist${i}" class="mt-[1vh]"></div>
        </div>`;
    } else if (task.action === 'focusBlock') {
      actionHtml = `
        <button id="focusBlock-start-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Focus block completed' : 'Start focus block'}">${isCompleted ? 'Completed' : 'Start'}</button>
        <div id="focusBlockTimer${i}" class="hidden ml-[1vw]">
          <span id="focusBlockDisplay${i}" class="text-[0.9rem]"></span>
          <button id="focusBlock-stop-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-red-500 text-white rounded hover:bg-red-600" aria-label="Stop focus block">Stop</button>
        </div>`;
    } else if (task.action === 'microTask') {
      actionHtml = `
        <button id="microTask-start-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Micro-task completed' : 'Start micro-task'}">${isCompleted ? 'Completed' : 'Start'}</button>
        <div id="microTaskTimer${i}" class="hidden ml-[1vw]">
          <span id="microTaskDisplay${i}" class="text-[0.9rem]"></span>
          <button id="microTask-complete-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-green-500 text-white rounded hover:bg-green-600" aria-label="Complete micro-task">Done</button>
          <button id="microTask-stop-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-red-500 text-white rounded hover:bg-red-600" aria-label="Stop micro-task">Stop</button>
        </div>`;
    } else if (task.action === 'workStretch') {
      actionHtml = `
        <button id="workStretch-start-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Work-stretch completed' : 'Start work-stretch'}">${isCompleted ? 'Completed' : 'Start'}</button>
        <div id="workStretchTimer${i}" class="hidden ml-[1vw]">
          <span id="workStretchDisplay${i}" class="text-[0.9rem]"></span>
          <button id="workStretch-stop-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-red-500 text-white rounded hover:bg-red-600" aria-label="Stop work-stretch">Stop</button>
        </div>`;
    } else if (task.action === 'accountability') {
      actionHtml = `
        <input id="accountability${i}" class="ml-[1vw] p-[0.5rem] border rounded text-[0.9rem] dark:bg-gray-700 dark:text-gray-200" placeholder="Enter buddy's contact info..." aria-label="Enter accountability buddy's contact info">
        <button id="accountability-complete-${i}" class="ml-[1vw] py-[0.5rem] px-[1rem] bg-blue-500 text-white rounded hover:bg-blue-600 ${isCompleted ? 'disabled completed' : ''}" aria-label="${isCompleted ? 'Accountability task completed' : 'Mark accountability task as completed'}">${isCompleted ? 'Completed' : 'Mark as Completed'}</button>`;
    }
    div.innerHTML = `
...
function addChecklistItem(index) {
  ...
  if (input.value.trim()) {
    ...
    div.className = 'flex items-center space-x-[1vw] mt-[0.5vh]';
    div.innerHTML = `
      <input type="checkbox" class="w-[1.5rem] h-[1.5rem]" aria-label="Mark checklist item as done">
